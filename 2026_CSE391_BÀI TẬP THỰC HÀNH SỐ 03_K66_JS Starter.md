...
- Khi trang web được tải, JavaScript trong file main.js được thực thi tự động.
Lệnh console.log() sẽ in thông báo ra bảng Console của trình duyệt để lập trình viên kiểm tra chương trình.
---
...
Trên server với Node.js
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: Dùng để tạo cấu trúc và nội dung của trang web
> - CSS: Dùng để thiết kế giao diện, màu sắc, bố cục cho trang web
> - JavaScript: Dùng để tạo tương tác, xử lý logic và thay đổi nội dung trang web một cách động.
...
- Xin chào, mình là Hai, năm nay mình 20 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
-Quên dấu ;

Thường không bị lỗi vì JavaScript có cơ chế tự thêm dấu ; 
Nhưng trong một số trường hợp có thể gây lỗi logic
Quên dấu +
Trình duyệt sẽ báo lỗi trong Console:
...
-Điều thú vị nhất là Console có thể chạy trực tiếp các dòng lệnh JavaScript và hiển thị kết quả ngay lập tức, giúp kiểm tra code nhanh mà không cần sửa file hay reload nhiều lần.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
-Em gặp lỗi SyntaxError do quên dấu + khi nối chuỗi trong console.log(). Trình duyệt hiển thị thông báo lỗi trong Console. Sau đó em đọc thông báo lỗi và sửa lại code bằng cách thêm dấu + cho đúng.
---
...
> a) Nếu `score = 9`, em dự đoán console sẽ in: gioi
> b) Nếu `score = 6`, em dự đoán console sẽ in: kha
...
> - Trường hợp `score = 9`: Dự đoán vs Thực tế: true
> - Trường hợp `score = 6`: Dự đoán vs Thực tế: true
...
- Chương trình in "Giỏi" khi điểm số lớn hơn hoặc bằng 8.
> - Khi nào chương trình in `"Yếu"`?
-Chương trình in "Yếu" khi điểm số nhỏ hơn 5.
> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):
- Cấu trúc if/else giống như một ngã rẽ trong cuộc sống. Chương trình sẽ kiểm tra một điều kiện.
Nếu điều kiện đúng thì chương trình đi theo nhánh if.
Nếu điều kiện sai thì chương trình đi theo nhánh else.
---
...
>   - Tham số (parameters): các giá trị truyền vào hàm
>   - Thân hàm (body): phần code bên trong { } để xử lý logic
>   - Giá trị trả về (return): kết quả mà hàm trả về sau khi thực hiện, ví dụ return "Giỏi"
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
- Giảm lặp lại code
- Dễ quản lý và sửa lỗi
- Có thể tái sử dụng nhiều lần
- Giúp chương trình gọn gàng và dễ hiểu hơn
---
...
- Phần if/else trong hàm xepLoai(avg) vì phải sắp xếp các điều kiện đúng thứ tự để chương trình trả về kết quả chính xá
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
- Em đọc lại ví dụ trong bài, thử chạy nhiều lần trong Console, sửa lại điều kiện if/else và tra thêm trên Google để hiểu rõ cách hoạt động của hàm và điều kiện.
---
...
- Lệnh này dùng để tìm và lấy phần tử HTML có id = "status" trong trang web để JavaScript có thể thay đổi nội dung hoặc thuộc tính của nó.
...
- input → liên quan đến nhập dữ liệu
- click → liên quan đến nhấn chuột
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì?
- Khi xoá hết nội dung, ô input không còn giá trị, nên dòng greeting sẽ trống hoặc không hiển thị gì.
...
>   DOM là cấu trúc dạng cây của trang HTML, trong đó mỗi thẻ HTML được xem như một đối tượng (node). JavaScript có thể truy cập, thay đổi nội dung, kiểu dáng hoặc hành vi của các phần tử đó thông qua DOM.
> - Nêu 1 ví dụ “thao tác DOM” trong bài (ghi lại 1 dòng lệnh cụ thể).
- statusEl.textContent = "Xin chào! Đây là nội dung được thay đổi bằng JavaScript.";
- Dòng lệnh này dùng JavaScript để thay đổi nội dung văn bản của phần tử HTML có id là status.
...
Khi người dùng nhấn vào nút, trình duyệt sẽ hiển thị thông báo Hello bằng alert().
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
- Điểm khác nhau về cú pháp
JavaScript thuần
Dùng document.getElementById("btnHello") để lấy phần tử HTML theo id.
Dùng addEventListener("click", ...) để gắn sự kiện click.
jQuery
Dùng $("#btnHello") để chọn phần tử theo id (ngắn gọn hơn).
Dùng .on("click", ...) để gắn sự kiện.
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Đơn giản hóa việc thao tác DOM và xử lý sự kiện (chọn phần tử, thay đổi nội dung, style…).
>   2. Viết JavaScript nhanh và ngắn gọn hơn, giúp code dễ đọc và dễ phát triển web.
...
- Em tò mò nhất về cách JavaScript tương tác với HTML thông qua DOM và xử lý sự kiện như click hoặc input.
> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …)?
- Em muốn thử làm nút đổi theme (dark mode / light mode) hoặc bộ đếm số lần người dùng nhấn nút.
